...
io.on('connection', (socket) => {
  ...
  socket.on('admin_correct', ({ roomId }) => {
    ...
    if (room.playerStats[winner.name]) {
      room.playerStats[winner.name].score += 1;
      room.gameState.scores[winner.team] = (room.gameState.scores[winner.team] || 0) + 1;
    }
...
server.listen(PORT, async () => {
  ...
  try {
    if (NGROK_AUTHTOKEN && NGROK_AUTHTOKEN !== 'YOUR_NGROK_AUTHTOKEN_HERE') {
      const listener = await ngrok.connect({
        addr: PORT,
        authtoken: NGROK_AUTHTOKEN,
      });
      globalNgrokUrl = listener.url();
      console.log(`Ngrok URL: ${globalNgrokUrl}`);
      launchUrl = `${globalNgrokUrl}/home.html`;
    } else {
      console.warn('⚠ Ngrok authtoken not set. Skipping tunnel.');
      globalNgrokUrl = `http://localhost:${PORT}`;
    }
  } catch (err) {
